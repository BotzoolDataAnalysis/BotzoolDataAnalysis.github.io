...
Almost publication-ready, except the grey background is not very nice. It might be easily removed by adding a different `theme`. One of our favourites is `theme_bw(),` meaning "black and white", but you can experiment with [different ones](https://ggplot2.tidyverse.org/reference/ggtheme.html) or even define your own.
...
8.  Explore the relationship between the Pokémon attack and defense power. Distinguish different Pokémon types. Select just three Pokémon types that you like to display. \* When using different colors for many categories, the plot gets quite messy, try to come up with a solution to distinguish the Pokémon types more clearly in the plot.
