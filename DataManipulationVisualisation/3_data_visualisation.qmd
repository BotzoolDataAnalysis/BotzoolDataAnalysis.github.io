...
Now it is clear that species identity is really important and the relationship between the bill dimensions is in fact positive.
...
1.  scatterplot with different data, relationship of two numerical variables, add groups (iris, species richness in vegetation plots vs environmental variables)

2.  histogram, other way to display distibution of numerical variable? (density plot, violin plot)

3.  boxplots for different data (iris, species richness in vegetation plots vs vegetation types, hydro?)

4.  three or more variables, how to deal with them? Propose a solution (colour/fill + shape, faceting)

5.  playing with `scale` - set different colours7

6.  playing with `theme` - remove legend, axis labels, background grid

    always save the resulting plot
