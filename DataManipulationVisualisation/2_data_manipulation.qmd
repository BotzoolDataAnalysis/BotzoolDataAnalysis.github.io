...
selected variables and filter them to a few defined cases. We will
prepare new variables and rearrange the data according to them. We will
further train on importing and exporting the data.

Again we will use the data from the training project that you downloaded
in the chapter 1. Before you start, I recommend to **create a new
script** for this chapter and to **clear objects from the workspace**
(use icon of the broom to clean your working environment).
...
```{r}
data <- read_csv("data/forest_understory/Axmanova-Forest-understory-diversity-analyses.csv") %>% select(PlotID, ForestType, ForestTypeName, Biomass,pH_KCl)
```
...
match the **exact value**, so we have to use **EQUAL operator** `==`.
...
conditions**. Here, we select all plots of the specified types and use
further condition to include only those with biomass above specified
level.

All of the conditions need to be valid for the case to be included in
the selection, because we included **operator AND** `&`.
...
Here we use the same approache, but specify conditions for three
variables at once.
...
condition is met. For example either high herb-layer biomass or high pH,
which are two different ways how to estimate relatively productive
forest site.
...
We can **combine AND and OR** also in one filter step. For xample we
...
For example here I define the conditions for selection category "yes"
and the rest will be "no" without any need for other definitions of this
second group, this is done by using the TRUE \~ .
...
change names of the categories. Check with view () if the other
...
Now you can use the same lines but use the mutate directly to change the
ForestTypeName.
...
numerical to character. To work with multiple variables at once, e.g.,
...
find out more in the readr cheatsheet.
...
Write excel_csv file, which can help to easily open the file in Excel
and will keep the coding you need.
...
6\. Using `case_when()` define new variable called productivity, with
three categories: low, medium, and high, with tresholds of x\<=60,
...
prevails among these top 15? \>\> \*How many rows/cases you actually got
and do you know why?
