...
4.  Combine the two plots using `p1 / p2`. What is the difference compared to `p1 + p2`? Add annotations 'A' and 'B' to the panels. \* Modify the legend so that there is only one legend with coloured points at the bottom.
5.  Use the dataset `Axmanova-Forest-understory-diversity-analyses.xlsx` and create four boxplots comparing Ellenberg-type indicator values in different forest types. Combine all four plots together and add annotations to individual panels. Do not show any legend and remove the title for the x-axis. \* Turn the axis tick labels by 45°.
6.  plot xx
7.  plot yy
8.  recreate a plot
9.  Save all plots you created so far to the `plots` folder.
