...
We will now look at three types of functions:

- Vector functions - they take one or more vectors as input and return a vector as output

- Data frame functions - they take a data frame (or tibble) as input and return a data frame (or tibble) as output

- Plot functions - they take a data frame (or tibble) as an input and return a plot as output
...
### 8.1.1 Vector functions

::: {.callout-note appearance="minimal"}
A **vector** is a one dimensional data type, you can imagine it as one column of a data frame or tibble. It is a collection of values of the same type, e.g., character, integer, logical.

In R, we can easily create a vector using `c()`. Based on the values, R will automatically assign the type to the vector. For example, `fruits` will be a character vector:

`fruits <- c('banana', 'apple', 'orange', 'pear')`

`numbers` a numeric vector:

`numbers <- c(1, 2, 3, 4, 5)`

and `truefalse` a logical vector:

`truefalse <- c(TRUE, TRUE, FALSE, TRUE)`
:::

Vector functions will be most useful for us when used inside `mutate()` to modify a column in our dataset. Actually every function we used in `mutate()` so far (e.g., `round()`, `sqrt()`, `ifelse()`) was a vector function predefined in some package.
...
A specific case of vector functions are summary functions that take a single vector and return a single values (e.g., `min()`, `max()`, `mean()`). These functions might be used in a `summarise()` call. For example, we can calculate the difference between the minimum and maximum values of the given variable:
...
plot_species(penguins, 'Chinstrap')
plot_species(penguins, 'Gentoo')
```
...
The for loop takes one variable from a given sequence, runs the code inside `{}` for this variable and then moves to the next variable in the sequence. A basic example might be printing elements from a vector of fruits:

```{r}
fruits <- c('apple', 'banana', 'orange', 'pear')
for (i in fruits) {
  print(i)
}
```

The for loop takes one value from the vector, prints it, then takes the next one, prints it, and so on till the end of the sequence.

Another example might be a basic numerical operation with each element of a list of numeric values:
...
## 8.3 purrr and working with nested dataframes

### 8.3.1 Basic usage of `purrr`
...
The structure of the `map()` function may already be familiar to you, as it resembles that of the `across()`. Note the difference between the output of the for loop and `map()`. In the output of the for loop, we see `[1] 'apple'`, whereas `map()` returns `[[1]] [1] 'apple'`. This is because the `map()` function returns a **list**. We can save the output to an object and look at the structure.

```{r}
map_out <- map(fruits, ~.x)
```
...
We can also rewrite our for loop used for creating plots of the relationship between penguin bill length and bill depth separately using the function from 7.1.3 in a `map()` function:
...
Let's begin with the task of reading multiple files simultaneously. Imagine you have multiple files with the same structure, but for some reason, they are stored in separate files. As example data, we will use the [gapminder](https://www.gapminder.org/data) dataset, which provides values for life expectancy, GDP per capita, and population size. The data we have in the `gapminder` folder ([Link to the Github folder](https://github.com/BotzoolDataAnalysis/BotzoolDataAnalysis.github.io/tree/main/DataManipulationVisualisation/data)) is divided by year

```{r}
list.files('data/gapminder/')
```

We now want to load them all and combine them into a single tibble. We could do that by copy-pasting the `read_csv()` function twelve times, but there is a more elegant way. First, we will have to create a vector of paths to the individual files. In addition to what we know already, we added two more arguments to `list.files()`. `pattern` specifies a common pattern of the files we want to list, in our case `.csv` to ensure we will load only csv files in case there is some other file, e.g. a readme, in the folder. `full.names = T` returns a full path to the file from the project directory needed to read the file.
...
We can now check the structure of object we just created:
...
In the data column we now have a tibble with 12 rows and 5 columns for each country. We can access one of them, for example, Austria, using the `[[]]`.
...
5.  Write a function with two arguments, which divides value `x` by a second value `y` (`x/y`).

6.  Write a function that calculates the heat load index using measured values of slope and aspect in a vegetation plot. You will need the following formula [@mccune_equations_2002]:
...
7.  Use the function to calculate the heat load index for vegetation plots in basiphilous grasslands (use the same data as in exercise 4). \* Calculate species richness of the plots and plot the relationship between the heat load index and species richness. Select only plots sampled in 2022 which are more precisely located. (Use the species data `basiphilous_grasslands/basiphilous_grasslands_S_Moravia_species.csv`, transform them to the long format, calculate the number of species per plot and join this information to the header data before you start plotting).

8.  Write a function to calculate the logistic population growth using the formula $N_t = \frac{K}{1 + \left( \frac{K - N_0}{N_0} \right) e^{-r t}}$, where $N_0$ is the initial population, K the carrying capacity, r an intrinsic growth rate and t time. Run the function for the values $N_0 = 10$, K = 100, r = 0.3 and time varies from 1 to 20.

    `K / (1 + ((K - N0) / N0) * exp(-r * t))`

9.  \* Write a function that calculates the Shannon diversity index using the formula $H' = -∑p_i ln(p_i)$. Use the function to calculate Shannon diversity of each plot in the sand vegetation dataset.

10. Take the Pokémon dataset (`read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/main/data/2025/2025-04-01/pokemon_df.csv')`) and visualise the distribution of defense power of water-type Pokémon. Turn this code into a function that helps you draw a histogram of defense power for different Pokémon types. \* Try to generalize the code so that you can plot a distribution of any numerical variable.

    **Chapter 9**

11. Use the function in a for loop and create plots of the distribution of defense power for all Pokémon types. \* Save all plots in a `plots` folder.

12. Write a for loop that takes a numeric vector of numbers 1-10, adds 1 to each element and then prints it.

13. Write a for loop that takes penguin species names and for each element prints for example: `Species: Adelie`.

14. Use a function from the `purrr` package to calculate the logaritm of each value in the following list: `test_list <- list(0, 1, 4, 8, 10)`. Save the output first as a list and second as a numeric vector. Compare the differences.

15. Take the following numeric vectors and use a function from the `purrr` package to multiply the first one by the second one. `num1 <- list(1, 2, 3, 4, 5, 6)`, `num2 <- list(7, 8, 9, 10, 11, 12)`. Save the output first as a list and second as a numeric vector. Compare the differences.

16. Use the function from exercise 10 and create plots of the distribution of defense power for all Pokémon types using functions from the `purrr` package.

17. \* In this exercise, we will use data of the 10 most common frog species recorded within the Australian [FrogID initiative](https://www.frogid.net.au/explore). The original data comes from the [TidyTuesdays repository](https://github.com/rfordatascience/tidytuesday/blob/main/data/2025/2025-09-02/readme.md), but we modified it slightly for this exercise and stored it in the `frogs` folder of our [repository](https://github.com/BotzoolDataAnalysis/BotzoolDataAnalysis.github.io/tree/main/DataManipulationVisualisation/data). The data are stored in multiple csv files, one for each species. Load all these files using a single pipeline and save to your environment as a single tibble. Store the species names in a column in the final tibble.

18. \* Create a bar plot showing which frog species are most often recorded (to order the species from the most to the least common, use a function from the `forcats()` package). Add colour distinction of records from different territories (column `stateProvince`).

19. \* Save the frog data as separate csv files by territory, where the observations were made in a folder `frogs_territory`.

20. \* Use the Pokémon dataset, nest it by Pokémon type and calculate linear models for the relationship between defense and attack power (`lm(defense ~ attack)`) of each Pokémon type using a `map()` function within `mutate()`. Use the `tidy()` function to extract coefficients and p-values from the model. Unnest the list column where you saved the results of `tidy()`. For each Pokémon type, filter only statistics for the attack coefficient, keep only significant results (p \< 0.05) and arrange the dataset according to the coefficient from the linear model from Pokémon types where the relationship between defence and attack power is the strongest to those with the weakest relationship.

21. \* Take the code for visualisation of Ellenberg-type indicator values in different forest types (Exercise 5 in Chapter 6) and turn it into a function. Draw the boxplots for four different Ellenberg-type indicator values using this function. Use a for loop or `purrr` functions to make these four plots. Combine them and save them in the `plots` folder.
