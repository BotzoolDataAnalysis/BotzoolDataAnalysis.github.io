---
title: "6 + 7 Advanced Visualisation"
...
But we can also use a predefined colour scale, e.g. from [colour brewer](https://colorbrewer2.org) using `scale_fill_brewer()`. We can also modify the labels in the legend by using the argument `labels`. Let's say we want to add the word 'Island' to each label:
...
We used `type = 'qual'` inside `scale_color_brewer()` to use colours appropriate for discrete values. You can experiment with many more such colour scales that are already available, e.g., [here](https://r-charts.com/color-palettes/).
...
- [`gordi`](CommunityEcology/gordi_vignette.qmd) a ggplot-based package for making ordination plots, developed in our group

- [`ggtree`](https://www.bioconductor.org/packages/release/bioc/vignettes/ggtree/inst/doc/ggtree.html) for visualisation of phylogenetic trees
...
8.  Combine the two plots from exercises 2 and 3 using `p1 / p2`. What is the difference compared to `p1 + p2`? Add annotations 'A' and 'B' to the panels. Modify the legend so that there is only one legend with coloured points at the bottom.

9.  Calculate mean, minimum and maximum of penguin bill length for each species and visualize it using `geom_pointrange()`. Order the species according to their mean bill length.

10. Take the plot from exercise 6, order the categories in legend according to the column `forest_type`. Move legend to the bottom of the plot.

11. Use the dataset `Axmanova-Forest-understory-diversity-analyses.xlsx` from the folder `forest_understory` ([Link to the Github folder](https://github.com/BotzoolDataAnalysis/BotzoolDataAnalysis.github.io/tree/main/DataManipulationVisualisation/data)) and recreate the following plot:
    ...
    forest_herb <- read_xlsx('data/forest_understory/Axmanova-Forest-understory-diversity-analyses.xlsx')

    plot_a <- forest_herb |> 
      clean_names() |> 
      mutate(forest_type_name = fct_reorder(forest_type_name, forest_type)) |>
      ggplot(aes(forest_type_name, eiv_light, fill = forest_type_name)) + 
      geom_boxplot(color = 'black', show.legend = F) +
      theme_bw()+
      theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title.x = element_blank()) +
      labs(y = 'Ellenberg-type Light')

    plot_b <- forest_herb |> 
      clean_names() |> 
      mutate(forest_type_name = fct_reorder(forest_type_name, forest_type)) |>
      ggplot(aes(forest_type_name, eiv_moisture, fill = forest_type_name)) + 
      geom_boxplot(color = 'black', show.legend = F) +
      theme_bw()+
      theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title.x = element_blank()) +
      labs(y = 'Ellenberg-type Moisture')

    plot_c <- forest_herb |> 
      clean_names() |> 
      mutate(forest_type_name = fct_reorder(forest_type_name, forest_type)) |>
      ggplot(aes(forest_type_name, eiv_nutrients, fill = forest_type_name)) + 
      geom_boxplot(color = 'black', show.legend = F) +
      theme_bw()+
      theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title.x = element_blank()) +
      labs(y = 'Ellenberg-type Nutrients')

    plot_d <- forest_herb |> 
      clean_names() |> 
      mutate(forest_type_name = fct_reorder(forest_type_name, forest_type)) |>
      ggplot(aes(forest_type_name, eiv_soilreaction, fill = forest_type_name)) + 
      geom_boxplot(color = 'black', show.legend = F) +
      theme_bw()+
      theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title.x = element_blank()) +
      labs(y = 'Ellenberg-type Soil Reaction')
...
12. Use the Pokémon dataset (`read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/main/data/2025/2025-04-01/pokemon_df.csv')`), calculate the mean defense power for each Pokémon type and draw a barplot showing the mean defense power for each type. Order the bars from the highest to the shortest. \*Colour the bars according to the colour defined for each Pokémon type in the column `color_1`. Add errorbars showing the minimum and maximum for each type.

13. Select electric Pokémons and visualise the relationship between their weight and height. Use their names as text labels instead of points. Do the same for ice Pokémons and then combine both plots together. Add annotations 'A' and 'B' to the panels.

14. Modify axis labels, legend, colours, etc. of all plots you created so far, so that you like their appearance and save them to the folder `plots`.
