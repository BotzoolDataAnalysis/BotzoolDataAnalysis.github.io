...
![](images/clipboard-509340872.png)
...
In this case I want to work with the **spe dataset**, were all records of individual species are merged across layers.

```{r}
spe <- read_csv ("data/turboveg_to_R/spe_merged_covers_across_layers.csv")
```
...
status <- read_excel("data/turboveg_to_R/Pladias-taxony-puvod-invazni-status-cerveny-seznam-ochrana-2023-11-06.xlsx") %>% 
  ...
  mutate(origin = case_when(
    ...
    origin %in% c("archaeofyt/neofyt", "archaeofyt") ~ "arch",
    origin == "neofyt" ~ "neo",
    TRUE ~ NA_character_))%>% 
  mutate(redlist = case_when(redlist== "taxon není zařazen do Červeného seznamu" ~ NA))
```
...
plant_height <- read_excel("data/turboveg_to_R/Vyska.xlsx") %>% 
  ...
  mutate(mean_height = ((height_min+height_max)/2))
```
...
For **env dataset** I want to import the clean version which was saved in this script

```{r}
env<- read_csv("data/turboveg_to_R/env.csv")
```

and I want to add some more variables that were measured separately.
...
table with min, mean, max plant height, biomass per veg type

boxplots of ellenberg-type indicator values

two in one\> soil ph against species richness, different coulours for different forest typesbiomass against species richness, different coulours for different forest types

\% of redlist species, alien species in veg type

```{r}
# statistics <- spe %>% 
#   group_by(releve_nr) %>% 
#   summarize(covertotal_overlap = combine_cover(cover_perc)) %>%
#   left_join(status) %>% 
#   filter(!is.na(redlist))%>%
#   group_by(releve_nr)%>%
#   summarise(cover_perc)
```
