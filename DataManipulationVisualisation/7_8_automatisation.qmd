...
The `map()` function works similarly to `across()`, but instead of doing something to each column in a data frame or tibble, it does something to each element of a vector. It is an analogy of a for loop, the function takes each element of a sequence and applies a function to it. Let's look at the difference between the for loop and `map()` in the following example:
...
The structure of the `map()` function might be already familiar to you, because it resembles that of `across()`. Note the difference between the output of the for loop and `map()`. In the output of the for loop, we see `[1] 1`, whereas `map()` returns `[[1]] [1] 1`. This is because the `map()` function returns a **list**.
