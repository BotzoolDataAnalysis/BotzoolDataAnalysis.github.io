...
Note that in the examples above you are not really rewriting the data you have imported. Here the "data %>%" means, that you are just trying how it would look like if you apply the following steps to data. To really change it you would have to assign your result into any new dataset. \- \- data2<- data1 %>% ... #defining first the new dataset
\- data<-data %>% ...    #rewriting the dataset (the same would be done by data %<>% data)
