...
Note that function that works in an opposite direction is called `separate`.
...
2\. Use **Forest understory environmental data** `Axmanova-Forest-env.xlsx`\> prepare a new variable which will combine Forest type code and name (unite) \> calculate mean values of biomass for these forest types (group_by, summarise) \> prepare a graph with boxplots of forest type and biomass (ggplot, geom_boxplot). \*you can also play more with the data, calculate min, mean and max values for biomass and soil pH at once.

3\. Train transformation of long to wide with another dataset **Acidophilous grasslands** namely the species file called `SW_moravia_acidgrass_species.csv`. Import the data and check structure \> prepare new variable with species name and vegetation layer \> transform the format to wide.

4\. Work with dataset **Lepidoptera**, namely `spe_matrix_MSvejnoha`, which is the species file with counts of moths in forest steppe localities. Import the data and check the structure \> transform to long format \> count number of species at each site (not individuals, just unique names using presence/absence data) \> create barplot to visualise the differences.

5\. We will use the **iris dataset** integrated in R. Use glimpse(iris) to check the structure \> if needed change the format to tibble using `as_tibble` function (iris.data\<- iris %\>% as_tibble()) \> calculate median values for selected parameters within different iris species.

6\. “*Tidy datasets are all alike, but every messy dataset is messy in its own way.*” Hadley Wickham Import `Example4` from the **Messy data** and try to find duplicate rows.

7\. \* Import `Example2` from the Messy data and try to make the data tidy with the use of `separate` function. This works in an opposite direction to `unite`.
