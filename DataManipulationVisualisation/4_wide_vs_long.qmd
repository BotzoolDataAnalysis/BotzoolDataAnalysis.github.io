...
We need to remove the empty rows. Because we filled them before with zeros, we will filter out the rows with cover=0.
...
Sometimes we have data with NAs (not zeros) here it is very usefull to use argument **values_drop_na** =TRUE
